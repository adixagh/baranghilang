<div class="col-sm-9 products">
          <div class="row">

          <?php
						$QueryBarang = mysqli_query($koneksi, "SELECT * FROM tbl_baranghilang p LEFT JOIN tbl_kategori k on p.id_kategori = k.id_kategori where k.id_kategori = $data");
						while ($show = mysqli_fetch_array($QueryBarang)) {
						?>
            <div class="col-sm-6 col-md-4 product">
            
              <a href="#favorites" class="favorites" data-favorite="inactive"><i class="ion-ios-heart-outline"></i></a>
              <a href=""><img src="admin/upload/<?= $show['gambar']; ?>" /></a>

              <div class="content">
                <h1 class="h4"><?= $show['nama']; ?></h1>
                <p class="price"><?= $show['nama_kategori']; ?></p>
                <label><?= $show['tempat_terakhir']; ?></label>

                <a href="detail.php?id_baranghilang=<?php echo $show['id_baranghilang']; ?>" class="btn btn-link"> Details</a>
                <button class="btn btn-primary btn-rounded btn-sm"> <i class="ion-bag"></i> Add to cart</button>
              </div>
            </div>
            <?php } ?>
            
            

          </div>

          <nav>
            <ul class="pagination">
              <li>
                <a href="#" aria-label="Previous">
                  <span aria-hidden="true"><i class="ion-ios-arrow-left"></i></span>
                </a>
              </li>
              <li class="active"><a href="#">1</a></li>
              <li><a href="#">2</a></li>
              <li><a href="#">3</a></li>
              <li class="disabled"><a href="#">..</a></li>
              <li><a href="#">10</a></li>
              <li>
                <a href="#" aria-label="Next">
                  <span aria-hidden="true"><i class="ion-ios-arrow-right"></i></span>
                </a>
              </li>
            </ul>
          </nav>
        </div>
        <!-- /// -->
      </div>
    </div>