    ...
    if ($tipe_file == "image/jpeg" || $tipe_file = "image/png" || $tipe_file = "image/jpg") {
        if ($ukuran_file <= 1000000) {
            if (move_uploaded_file($tmp_file, $path)) {
                $QuerySimpan = mysqli_query($koneksi, "INSERT INTO tbl_barangtemuan (id_kategori, nama, gambar, deskripsi, tempat_terakhir, no_telp, tanggal,status, id_member) VALUES ('$kategori','$namaBarang','$namaGambar','$deskripsiBarang','$tempatTerakhir','$nomerTelepon','$tanggal','$status','$id_member')");
                if ($QuerySimpan) {
                    echo "
                    <script>
                    ...
                    <script>
                        ...
                        window.location = 'http://localhost/barangtemuan/inputbarangtemuan.php';
                    </script>";
                }
            } else {
                echo "
